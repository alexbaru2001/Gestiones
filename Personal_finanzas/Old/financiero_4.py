...
def plot_explicacion_gastos(df_gastos, df_ingresos, categorias_fijas=None):
    """
    Genera explicación de gastos (figuras + estadísticas) SIN imprimir ni mostrar.

    Requiere que exista: resumen_gastos(df_gastos, df_ingresos) -> pivot mensual
    con columnas por categoría (p.ej. alimentacion, transporte, etc.)

    Devuelve:
      - pivot_explicacion: DataFrame
      - stats: dict
    ...
    # % gasto fijo si existen esas columnas
    ...
    # DataFrame “largo” para algunos charts
    ...
    # 1) Barras apiladas + línea media
    fig1 = px.bar(
        df_reset,
        x="mes",
        y=list(pivot_explicacion.columns),
    ...
    # 2) “Distribución porcentual” (mejor normalizar a % para que sea realmente porcentual)
...
def plot_porcentaje_ahorro(gastos, ingresos):
    """
    Calcula porcentaje de ahorro mensual y devuelve:
