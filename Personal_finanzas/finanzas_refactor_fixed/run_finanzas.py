...
def main():
    ...
    data_dir = Path(os.getenv("FINANZAS_DATA_DIR", base_dir / "Data")).expanduser().resolve()
    excel_path = excel_path = (data_dir / "Inicio.xlsx")
    ...
    # Fallback: si no existe Data/Inicio.xlsx, intenta Inicio.xlsx en la carpeta del script
    if not excel_path.exists():
        alt = base_dir / "Inicio.xlsx"
        if alt.exists():
            excel_path = alt
        else:
            raise FileNotFoundError(
                f"No encuentro Inicio.xlsx. Probé: {data_dir/'Inicio.xlsx'} y {alt}. "
                "Solución: crea la carpeta Data con Inicio.xlsx o define FINANZAS_DATA_DIR."
            )
